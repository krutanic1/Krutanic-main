...
const AddUser = () => {
  ...
  const getTransactionIdList = async () => {
    try {
      ...
      setGetTransactionId(response.data.filter((item) => item.counselor?.toLowerCase() === medTeamName?.toLowerCase()));
    }
    catch (error) {
      console.error(error);
    }
  ...
  return (
    <div id="AdminAddCourse">
      ...
      <div className='adduser'>
        <h2>Hi <span className='text-red-600 font-bold' >{localStorage.getItem('medTeamName')}</span> kindly add Email ID below before sharing the onboarding link !!</h2>
