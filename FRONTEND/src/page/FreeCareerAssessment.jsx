...
const FreeCareerAssessment = () => {
  ...
  // ─── Step management ───────────────────────────────────────────────────────
  // Step 0: Landing + pre-payment form
  ...
  // ─── Mobile UPI Recovery ───────────────────────────────────────────────────
  // When user returns from Google Pay / PhonePe, the browser may have killed
  // this page's JS context. On reload, check if there's a pending payment.
  // CRITICAL FIX: we stay on THIS page instead of navigating to /paymentsucess,
  // which eliminates the state-loss race condition on Android.
  useEffect(() => {
    const recoverMobilePayment = async () => {
      try {
        ...
        if (isPaid && paymentData) {
          ...
          setCurrentStep(1); // Go directly to slot booking on same page
        } else {
          // Payment not yet captured after all retries — keep storage, let user retry manually if needed.
  ...
  // ─── Pre-payment submit (saves lead, then triggers payment) ────────────────
  ...
  const handlePayment = async (savedPrePaymentId, savedFormData) => {
    ...
    try {
      ...
      // Retry logic for order creation (handles cold starts / network hiccups)
      let order;
      ...
      // When the user comes back, the useEffect recovery will pick this up and
      // call setCurrentStep(1) directly — NO navigate() needed.
      ...
      const options = {
        ...
        handler: async function (response) {
          // Payment completed via JS callback (works on desktop; sometimes on mobile too)
          sessionStorage.removeItem('krutanic_pending_payment');
          try {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error.response?.data?.error === "SLOT_TAKEN") {
        ...
        setCurrentStep(1); // Back to calendar
  ...
  // ══════════════════════════════════════════════════════════════════════════
  return (
    <div className="text-zinc-300 font-['Inter'] min-h-screen selection:bg-indigo-500/30" style={bgStyle}>
      <Helmet>
