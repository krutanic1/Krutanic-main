...
const AdminVerticals = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [verticalsRes, managersRes, coursesRes] = await Promise.all([
        axios.get(`${API}/api/vertical?month=${filterMonth}&year=${filterYear}`, { withCredentials: true }),
        axios.get(`${API}/getbda`, { withCredentials: true }),
        axios.get(`${API}/getcourses`, { withCredentials: true })
      ]);
      
      setVerticals(verticalsRes.data);
      // Only keep users with MANAGER or LEADER designation who are strictly active
      const bdaManagers = managersRes.data.filter(u => 
        (u.status === "Active" || u.status?.toLowerCase().trim() === "active") && 
        (u.designation?.toUpperCase().trim() === "MANAGER" || u.designation?.toUpperCase().trim() === "LEADER")
      );
      setManagers(bdaManagers);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editMode) {
        await axios.put(`${API}/api/vertical/${selectedVerticalId}`, formData, { withCredentials: true });
        setSuccess("Vertical updated successfully!");
        setEditMode(false);
        setSelectedVerticalId(null);
      } else {
        await axios.post(`${API}/api/vertical/create`, formData, { withCredentials: true });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${API}/api/vertical/${id}`, { withCredentials: true });
    ...
    <div id="AdminAddCourse" style={{ padding: "20px", display: "flex", flexDirection: "column", gap: "30px" }}>
      <div className="admin-header" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
        ...
        <button 
          ...
          style={{ padding: "10px 15px", background: "#3b82f6", color: "white", border: "none", borderRadius: "5px", cursor: "pointer", fontWeight: "bold" }}
        >
          {showForm ? "Cancel" : "+ Create Vertical"}
        </button>
      </div>

      {error && <div className="alert alert-danger" style={{ color: "red", padding: "10px", backgroundColor: "#ffebee", borderRadius: "5px", marginBottom: "15px" }}>{error}</div>}
      {success && <div className="alert alert-success" style={{ color: "green", padding: "10px", backgroundColor: "#e8f5e9", borderRadius: "5px", marginBottom: "15px" }}>{success}</div>}

      {showForm && (
        <div className="admin-card" style={{ padding: "20px", background: "white", borderRadius: "8px", boxShadow: "0 4px 6px rgba(0,0,0,0.1)", marginBottom: "30px" }}>
          ...
          <div style={{ display: "flex", gap: "10px", alignSelf: "flex-start" }}>
            ...
            {editMode && (
              <button type="button" onClick={() => {
                ...
                setFormData({ name: "", managerId: "", targetValue: "", domains: [] });
                setSelectedVerticalId(null);
              }} style={{ padding: "10px 20px", background: "#f44336", color: "white", border: "none", borderRadius: "4px", cursor: "pointer", fontWeight: "bold" }}>
                Cancel
              </button>
            )}
          </div>
