...
const MyJob = () => {
  ...
  const handleSearch = async (page = 1) => {
    ...
    try { 
      ...
      const { jobId, cached } = submitResponse.data;

      // Step 2: If cached, fetch results immediately
      if (cached) {
        const resultsResponse = await axios.get(`${API}/api/jobs/${jobId}/results?page=${page}&limit=50`);
        const limitedJobs = resultsResponse.data.results.slice(0, MAX_JOBS);
        setJobs(limitedJobs);
        setCurrentPage(1);
        setPagination({
          page: resultsResponse.data.page,
      ...
      // Step 3: Poll for results (max 2 minutes)
      ...
      const pollInterval = setInterval(async () => {
        try {
          ...
          if (resultsResponse.data.status === 'completed' || resultsResponse.data.status === 'partial') {
            ...
            clearInterval(pollInterval);
            setError(resultsResponse.data.errorMessage || "Job search failed");
            setLoading(false);
