...
exports.createVertical = async (req, res) => {
  try {
    ...
    const existingVerticals = await Vertical.find({ domains: { $in: domains } });
    ...
    const newVertical = new Vertical({ name, managerId, targetValue, domains });
  ...
  try {
    ...
    const verticalsWithCount = await Promise.all(verticals.map(async (vertical) => {
      ...
      if (vertical.domains && vertical.domains.length > 0) {
         ...
         achievedCount = await NewStudentEnroll.countDocuments(query);
      }
...
exports.getVerticalsByManager = async (req, res) => {
  try {
    ...
    const verticals = await Vertical.find({ managerId }).populate("domains", "title");
    ...
    const verticalsWithCount = await Promise.all(verticals.map(async (vertical) => {
      ...
      if (vertical.domains && vertical.domains.length > 0) {
         ...
         const aggregation = await NewStudentEnroll.aggregate([
           { $match: query },
           {
             $group: {
...
exports.updateVertical = async (req, res) => {
  try {
    ...
    const existingVerticals = await Vertical.find({ _id: { $ne: id }, domains: { $in: domains } });
    if (existingVerticals.length > 0) {
      return res.status(400).json({ 
        error: "One or more selected courses are already assigned to another vertical." 
      });
...
exports.getVerticalEnrollments = async (req, res) => {
  try {
    ...
    const totalEnrollments = await NewStudentEnroll.countDocuments(query);
    const enrollments = await NewStudentEnroll.find(query)
    ...
    const availableCounselors = await NewStudentEnroll.distinct("counselor", counselorQuery);
