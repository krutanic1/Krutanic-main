...
router.get("/check-existing-med-enrollment", async (req, res) => {
...
/**
 * GET /getmedmonthlyrevenue
 * Aggregates revenue data by month including total, credited, and pending amounts.
 * Supports pagination.
 */
router.get("/getmedmonthlyrevenue", verifyAnyAuth, async (req, res) => {
  try {
    ...
    // execute aggregation
    const allMonthlyStats = await MedEnroll.aggregate(pipeline);
    ...
    console.error("Error in /getmedmonthlyrevenue:", error);
...
router.get("/get-med-enroll", verifyAnyAuth, async (req, res) => {
  ...
  try {
    ...
    if (studentenrollid) {
      ...
      StudentEnroll = await MedEnroll.findById(studentenrollid).lean();
      if (!StudentEnroll) {
        return res
          .status(404)
      ...
      if (all === "true") {
        StudentEnroll = await MedEnroll.find(query).sort({ createdAt: -1 }).lean();
        return res.status(200).json(StudentEnroll);
      } else {
        ...
        const totalItems = await MedEnroll.countDocuments(query);
        const totalPages = Math.ceil(totalItems / limitNum);

        StudentEnroll = await MedEnroll.find(query)
          .sort({ createdAt: -1 })
          .skip(skip)
          .limit(limitNum)
          .lean();
    ...
    console.error("Error in /get-med-enroll:", error);
    res.status(500).json({
...
router.post("/updatemedremark", verifyAnyAuth, async (req, res) => {
  const { remark, studentId, referRemark } = req.body;
  try {
    let existingStudent = await MedEnroll.findById(studentId);
    if (!existingStudent) {
...
router.put("/editmedstudentdetails/:_id", verifyAnyAuth, async (req, res) => {
  ...
  try {
    ...
    const studentData = await MedEnroll.findByIdAndUpdate(
      _id,
      {
        fullname,
        email,
...
router.post("/updateMedStudentStatus", verifyAnyAuth, async (req, res) => {
  const { studentId, status } = req.body;
  try {
    const student = await MedEnroll.findById(studentId);
    if (!student) {
...
router.post("/update-med-operation/:id", verifyAnyAuth, async (req, res) => {
  try {
    const { operationName, operationId } = req.body;
    const { id } = req.params;
    const objectId = new mongoose.Types.ObjectId(id);
    const updatedItem = await MedEnroll.findByIdAndUpdate(
      { _id: objectId },
      {
...
router.get("/med-enrollments", async (req, res) => {
  const { userEmail } = req.query;
  try {
    // Fetch all enrollments
    const enrollments = await MedEnroll.find({
      email: userEmail,
    }).lean();
...
router.get("/med-enrollments/:id/sessions", async (req, res) => {
  try {
    const { id } = req.params;
    const enrollment = await MedEnroll.findById(id).lean();
...
router.post("/updateprogress", async (req, res) => {
  ...
  try {
    ...
    let student = await MedEnroll.findByIdAndUpdate(
      enrollmentId,
...
router.post("/select-project", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    let student = await MedEnroll.findById(enrollmentId);
    if (!student) {
...
router.post("/update-project-progress", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    let student = await MedEnroll.findOneAndUpdate(
...
router.get("/databyopname", async (req, res) => {
  ...
  try {
    const OpName = await MedEnroll.find({ operationName: operationName })
      .sort({ createdAt: -1 })
      .lean();
...
router.get("/databybdaname", async (req, res) => {
  ...
  try {
    const students = await MedEnroll.find({ counselor: bdaName })
...
router.get("/getmeddailyrevenue", async (req, res) => {
  ...
  try {
    ...
    const dailyStats = await MedEnroll.aggregate(pipeline);
