...
import { DikshaantSender } from './models/DikshaantSender.js';
import { DikshaantTemplate } from './models/DikshaantTemplate.js';
import { DikshaantMail } from './models/DikshaantMail.js';
...
function requireMailblasterAuth(req, res, next) {
  ...
  // DEBUG BYPASS: Remove this before production if you need security
  req.mailblasterAuth = { email: 'admin@dikshaant.com' };
  return next();
...
async function fetchSendersWithAvailability() {
  const senders = await DikshaantSender.find()
    .select('user label blastedCount createdAt updatedAt')
    .sort({ createdAt: 1 })
    .lean();

  return senders.map((DikshaantSender) => {
    const availability = getSenderAvailability(DikshaantSender);
    return {
      ...toPublicSender(DikshaantSender),
      ...availability,
...
async function fetchTemplateArray() {
  const template = await DikshaantTemplate.findOne().lean();
...
async function incrementSenderBlastCounts(countMap) {
  ...
  if (senderIds.length) {
    try {
      ...
      await DikshaantSender.updateMany(
        {
          _id: { $in: senderIds },
      ...
      console.error('Failed to restore stale DikshaantSender counters:', err.message);
    }
  }
  ...
  try {
    await DikshaantSender.bulkWrite(ops, { ordered: false });
    invalidateReadCache();
  } catch (err) {
    console.error('Failed to update DikshaantSender blastedCount:', err.message);
  }
}
...
function shouldAutoReset20Hour(DikshaantSender, now = Date.now()) {
  const updatedAtMs = DikshaantSender?.updatedAt ? new Date(DikshaantSender.updatedAt).getTime() : NaN;
  const used = Number(DikshaantSender?.blastedCount || 0);
...
function getSenderAvailability(DikshaantSender, now = Date.now()) {
  ...
  const used = restored ? 0 : Number(DikshaantSender?.blastedCount || 0);
...
// ── Dikshaant Mail Blaster ───────────────────────────────────────────────────

app.get('/api/dikshaant-mails', async (_req, res) => {
  try {
    const mails = await DikshaantMail.find().sort({ createdAt: -1 }).lean();
...
app.post('/api/dikshaant-mails', async (req, res) => {
  try {
    const { emails } = req.body;
    if (!Array.isArray(emails)) {
      return res.status(400).json({ ok: false, message: 'Emails must be an array.' });
    }

    const operations = emails.map(email => ({
      updateOne: {
        filter: { email: String(email).trim().toLowerCase() },
        update: { $setOnInsert: { email: String(email).trim().toLowerCase(), createdAt: new Date() } },
        upsert: true
      }
    }));

    await DikshaantMail.bulkWrite(operations);
    res.json({ ok: true, message: `${emails.length} emails processed.` });
  } catch (err) {
    res.status(500).json({ ok: false, message: err.message });
  }
});

app.delete('/api/dikshaant-mails/:id', async (req, res) => {
  try {
    await DikshaantMail.findByIdAndDelete(req.params.id);
    res.json({ ok: true, message: 'Email deleted.' });
  } catch (err) {
    res.status(500).json({ ok: false, message: err.message });
  }
});
...
app.post('/api/mail-template', async (req, res) => {
  try {
    ...
    if (hasNewFormatInput) {
      ...
      const template = await DikshaantTemplate.findOneAndUpdate(
        {},
        { $addToSet },
...
app.post('/api/mail-template/item/update', async (req, res) => {
  try {
    ...
    const template = await DikshaantTemplate.findOne();
...
app.post('/api/senders', async (req, res) => {
  try {
    ...
    const sender = await DikshaantSender.create({ user: user.trim().toLowerCase(), pass, label: label || '' });
    invalidateReadCache();
    res.json({ ok: true, sender: toPublicSender(sender.toObject()) });
  } catch (err) {
...
app.delete('/api/senders/:id', async (req, res) => {
  try {
    await DikshaantSender.findByIdAndDelete(req.params.id);
    invalidateReadCache();
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ ok: false, message: err.message });
...
app.put('/api/senders/:id', async (req, res) => {
  try {
    ...
    const updatedSender = await DikshaantSender.findByIdAndUpdate(
      req.params.id,
      { $set: { pass } },
      { new: true, runValidators: true }
    ).lean();

    if (!updatedSender) return res.status(404).json({ ok: false, message: 'Sender not found.' });
    invalidateReadCache();
    res.json({ ok: true, sender: toPublicSender(updatedSender) });
  } catch (err) {
    res.status(500).json({ ok: false, message: err.message });
  }
});
...
app.get('/api/senders/:id/limit', async (req, res) => {
  try {
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
      return res.status(400).json({ ok: false, message: 'Invalid DikshaantSender id.' });
    }

    let sender = await DikshaantSender.findById(req.params.id).lean();
    if (!DikshaantSender) return res.status(404).json({ ok: false, message: 'DikshaantSender not found.' });

    const availability = getSenderAvailability(DikshaantSender);
    let autoResetAfter20Hours = false;

    if (availability.restored) {
      await DikshaantSender.updateOne({ _id: DikshaantSender._id }, { $set: { blastedCount: 0 } });
      sender = await DikshaantSender.findById(req.params.id).lean();
      invalidateReadCache();
    }

    // Check for 20-hour auto-reset for mails that didn't hit the limit
    if (shouldAutoReset20Hour(DikshaantSender)) {
      await DikshaantSender.updateOne({ _id: DikshaantSender._id }, { $set: { blastedCount: 0 } });
      sender = await DikshaantSender.findById(req.params.id).lean();
      invalidateReadCache();
    ...
    const currentAvailability = getSenderAvailability(DikshaantSender);

    res.json({
      ok: true,
      sender: DikshaantSender.user,
      ...
      updatedAt: DikshaantSender.updatedAt,
...
app.post('/api/validate-senders', async (req, res) => {
  try {
    const senders = await DikshaantSender.find().lean();
...
app.post('/api/generate-content', async (req, res) => {
  try {
    ...
    const fullPrompt = `You are an expert email campaign content writer for Dikshaant.
    Based on the following request, generate professional email campaign content for Dikshaant programs.
    RELEVANCE: The content must be focused on Dikshaant's identity. Mention "Dikshaant" as the provider of these elite professional training programs. 
    Ensure these mails are professional, high-value, and specifically NOT associated with any other brand like Krutanic.
    
    Make sure these mails won't be cited as spams.
...
Ensure the email provides genuine value or useful information about Dikshaant's professional ecosystem.

Include a polite closing and professional signature representing Dikshaant.

Request: ${prompt}

Generate content in the following JSON format with these fields:
- subjects: Array of exactly 50 different email subject lines mentioning Dikshaant where appropriate
- greetings: Array of exactly 50 different professional greetings (e.g., "Dear Professional,", "Hello,", "Hi there,")
- body_paragraphs: Array of exactly 50 different body paragraph variations about Dikshaant's advantage. Each variation must be concise. IMPORTANT: Each body paragraph MUST contain exactly 3 parts/sections separated by double line breaks. Each part should be a single efficient paragraph.
- links: Array of exactly 50 relevant call-to-action links or placeholders
- closings: Array of exactly 50 different professional closings (e.g., "Best from the Dikshaant Team,", "Sincerely,", "Warm regards,")
- signatures: Array of exactly 50 different signature variations explicitly for Dikshaant (e.g., "Dikshaant Program Management", "Team Dikshaant")

Example of a body paragraph with 3 parts for Dikshaant (keep each part efficient to ensure all 50 variations fit):
"Welcome to Dikshaant, where we redefine professional excellence through our advanced learning tracks. Our mission is to empower professionals with high-velocity skills for the modern economy.

Each Dikshaant program is architected by industry leaders and features intensive 1:1 mentorship. You'll engage with real-world case studies and gain an elite certification recognized globally.

Join the thousands of Dikshaant alumni who have transformed their careers and secured positions at top-tier firms. Explore our upcoming cohorts and take the next step in your professional journey with Dikshaant today."

Return ONLY valid JSON without any markdown formatting or code blocks. Ensure the JSON is properly terminated. Make the content professional, engaging, and suitable for a mail blast campaign. Ensure you generate EXACTLY 50 items for each field. Ensure the total output stays within token limits by keeping each paragraph variation concise and professional.`;
...
app.post('/api/blast', async (req, res) => {
  ...
  try {
    ...
    if (senderIdsProvided && !senderIds.length) {
      return res.status(400).json({ ok: false, message: 'Select at least one DikshaantSender account.' });
    }

    const senders = senderIdsProvided
      ? await DikshaantSender.find({ _id: { $in: senderIds } }).lean()
      : await DikshaantSender.find().lean();

    if (!senders.length) return res.status(400).json({ ok: false, message: 'No DikshaantSender accounts configured. Add senders first.' });

    const availableSenders = senders.filter((DikshaantSender) => getSenderAvailability(DikshaantSender).canUse);
    if (!availableSenders.length) {
      return res.status(400).json({
        ok: false,
        message: `All selected DikshaantSender accounts have reached the ${getConfiguredSenderLimit()} mail daily limit.`
      });
    }

    const smtpBase = req.body.smtp || defaultSmtp;
    // Always load the single mailtemp document; no templateId selection needed
    const baseTemplate = await DikshaantTemplate.findOne().lean();
    if (!baseTemplate) {
    ...
    // RECIPIENTS: Use provided recipients if available, otherwise use all from DikshaantMail collection
    let recipientsArr = normalizeEmailList(req.body.recipients);
    if (!recipientsArr.length) {
      const allDikshaant = await DikshaantMail.find().lean();
      recipientsArr = allDikshaant.map(d => d.email).filter(Boolean);
    }

    if (!recipientsArr.length) {
      return res.status(400).json({ ok: false, message: 'At least one recipient is required (either input manually or stored in Dikshaant database).' });
    }
    const recipients = recipientsArr;

    const singleShotBcc = req.body.singleShotBcc !== false;
    if (singleShotBcc) {
      if (!availableSenders.length) return res.status(400).json({ ok: false, message: 'No DikshaantSender account available.' });
      ...
      // Cache transporters per DikshaantSender index to avoid recreating on every batch
      ...
      try {
        ...
        return res.status(500).json({
          ...
          failed: recipients.map((to, idx) => {
            ...
            const senderLabel = DikshaantSender ? `${DikshaantSender.user} <${DikshaantSender.user}>` : 'N/A';
