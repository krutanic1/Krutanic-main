...
async function fetchInitialBootstrap() {
  if (!initialBootstrapPromise) {
    initialBootstrapPromise = apiFetch('/api/bootstrap').then((data) => {
      if (!data?.ok) {
...
function AdminMailBlaster({ authedEmail, onLogout }) {
  const [activeTab, setActiveTab] = useState('dikshaant'); // Default to dikshaant now
  ...
  async function loadInitialData() {
    const d = await fetchInitialBootstrap();
    if (d.ok) {
      const incomingSenders = d.senders || [];
      const incomingTemplates = d.templates || [];

      setSenders(incomingSenders);
      setEditPassById((prev) => Object.fromEntries(incomingSenders.map((s) => [s._id, prev[s._id] || ''])));
      setSelectedById((prev) => Object.fromEntries(incomingSenders.map((s) => [s._id, s.canUse ? (prev[s._id] ?? false) : false])));

      setTemplates(incomingTemplates);
      setSelectedTemplateId((prev) => {
        if (prev && incomingTemplates.some((template) => template._id === prev)) return prev;
        return incomingTemplates[0]?._id || '';
      });

      const tpl = incomingTemplates[0] || {};
  ...
  return (
    <div id="AdminAddCourse" className="mailblaster-admin">
      ...
      <div className="app-shell" style={{ padding: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap', borderBottom: '1px solid #e5e7eb', paddingBottom: 16, marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <img src="/image.png" alt="Logo" style={{ width: 40, height: 40, borderRadius: '50%', objectFit: 'contain' }} />
            <h2 style={{ margin: 0 }}>Dikshaant Mail Blaster</h2>
          </div>
